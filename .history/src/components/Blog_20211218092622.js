...
function Blog({blog}) {
    const {image,title,numberView,id,numberComment,numberLike,date,} = blog;
    ...
    return (
        <div className="blog-one__single ">
            ...
            <div className="blog-one__content">
                ...
                <h3>
                    <Link to="/vi/blog/details/du-an-nghi-duong-tai-phu-tho-noi-bat-nho-nhung-dich-vu-doc-dao-13">{title}</Link>
                </h3>
                <p className="multi-line-truncate">{url==='/vi'?"":`${title}`}</p>
            </div>
        </div>
    )
}
